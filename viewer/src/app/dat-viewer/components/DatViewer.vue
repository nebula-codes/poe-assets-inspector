<template>
  ...
  <div class="flex flex-1">
      ...
      <div :class="$style.rowsOverlay" :style="rowsOverlayStyle">
        <div class="absolute" :style="{ transform: `translate(0, ${renderItems.top}px)` }">
          <div
            ...
            :key="rowIdx"
            :style="{
              transform: `translate(0, ${i * LINE_HEIGHT}px)`,
              width: rowsNumberWidth,
              position: 'absolute',
            }"
            v-text="rowIdx"
          />
    ...
    <header-props @focus-editing-header="focusEditingHeader" />
  </div>
</template>
    ...
    setup(props, ctx) {
      ...
      const rowIndices = computed(() => {
        if (viewer.rowSorting.value) {
          ...
          return Array.from({ length: viewer.datFile.rowCount }, (_, rowIdx) => rowIdx)
        }
      })
      ...
      const rowsFullWidth = computed(() => rendering.getRowWidth(viewer.headers.value))
      const rowsFullHeight = computed(() => viewer.datFile.rowCount * rendering.LINE_HEIGHT)
      const rowsOverlayWidth = computed(() => rendering.rowsOverlayWidth(viewer.datFile.rowCount))
      const rowsWidth = computed(() => Math.max(0, paintWidth.value - rowsOverlayWidth.value))
      const rowsHeight = computed(() => Math.max(0, paintHeight.value - rendering.HEADERS_HEIGHT))
      ...
      const renderStats = computed(() => renderColStats(viewer.columnStats.value, viewer.datFile))
      ...
      watch([scrollPos, renderColumns, rowIndices, viewer.selectedRow, tablesLoadWatcher], () => {
        draw()
      })
      ...
      return {
        ...
        rowsOverlayStyle: computed(() => ({
          top: rendering.HEADERS_HEIGHT + 'px',
          width: rowsOverlayWidth.value + 'px',
          height: rowsHeight.value + 'px',
