<template>
  <div class="layout-column shrink-0" :style="{ width: showTree ? '300px' : undefined }">
    <div class="mb-px flex items-center p-1">
      <button title="Toggle tree visibility" :class="$style.treeToggle" @click="toggleTree">
    ...
    <template v-if="showTree">
      <div class="flex justify-end gap-x-1 border-b px-2 pb-1.5">
      ...
      <div v-if="!isIndexLoaded" class="p-2 text-center italic text-gray-500">
        Waiting for Index bundle...
      </div>
      <div v-if="isIndexLoaded && !tree.length" class="p-2 text-center italic text-gray-500">
        No results found
      </div>
      <virtual-scroll
        class="flex-1"
    ...
    async function handleTreeNav(item: TreeItem) {
      if (!item.isFile) {
        ...
        if (item.fullPath.endsWith('.datc64')) {
          ...
          FileSaver.saveAs(blob, item.fullPath.substring(item.fullPath.lastIndexOf('/') + 1))
        }
      }
    }
